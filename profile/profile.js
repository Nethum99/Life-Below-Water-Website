...
function displayStep() {
    ...
    stepC.innerHTML = `
        <h3>${step.title} - Step ${Index1 + 1}</h3>
        <p>${prompt}</p>
...
function nextPrompt() {
    ...
    if (currentStep.prompts[Index2] === "Age" ||
        currentStep.prompts[Index2] === "Completion Year" ||
        currentStep.prompts[Index2] === "Telephone Number") {
        if (!/^\d+$/.test(input)) {
...
function skipPrompt() {
    u_Data[steps[Index1].prompts[Index2]] = 'Not Answered';
    Index2++;
...
function calculateProgress() {
    let totalCompletedSteps = Index1 + (Index2 > 0 ? 1 : 0);
